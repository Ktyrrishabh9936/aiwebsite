...
export default function LeadQualificationPanel({ lead }) {
  ...
  const base = `/workspaces/${wsId}/crm/qualification`;
  const result = lead.qualification_call?.engine_result;
  useEffect(() => {
    let live = true;
    setSelected(lead.qualification_profile_id || ""); setHistory([]); setProfiles([]); setError("");
    Promise.all([api.get(`${base}/profiles`), api.get(`${base}/leads/${lead.id}/history`)]).then(([p, h]) => { if (live) { setProfiles(p.data.profiles); setHistory(h.data); } }).catch((e) => { if (live) setError(qualificationError(e)); });
    return () => { live = false; };
  }, [base, lead.id, lead.qualification_profile_id, lead.updated_at]);
  ...
  return <section className="rounded-xl border bg-card p-4 space-y-3">
    <div className="flex justify-between items-center gap-2"><h3 className="font-semibold">Qualification engine</h3><Link to={`/app/w/${wsId}/qualification`} className="text-xs text-primary">Configure profiles</Link></div>
    <label className="block text-xs text-muted-foreground">Profile for the next call<select aria-label="Lead qualification profile" disabled={busy} value={selected} onChange={(e) => assign(e.target.value)} className="block w-full mt-1 rounded-md border bg-background p-2 text-sm"><option value="">Use campaign / workspace default</option>{profiles.map((p) => <option key={p.id} value={p.id}>{p.product_name}</option>)}</select></label>
