...
export default function CrmReminders({ wsId, leadId, onOpenLead }) {
  ...
  const update = useCallback(async (id, payload) => {
    ...
    try { await api.patch(`${base}/reminders/${id}`, payload); setRevision((v) => v + 1); window.dispatchEvent(new Event("arevei:follow-up-changed")); return true; }
  ...
  return <section className="space-y-4" aria-label="Lead follow-ups">
    ...
    {(leadId || editing) && <form onSubmit={save} className="rounded-xl border bg-card p-4 space-y-3">
      <h4 className="text-sm font-medium">{editing ? "Edit follow-up" : "Plan a follow-up"}</h4>
      ...
      <label className="block text-xs">Follow-up date and time (your timezone)<input required type="datetime-local" value={draft.due_at} onChange={(e) => setDraft({ ...draft, due_at: e.target.value })} className={`${inputClass} mt-1`} /></label>
      ...
      <div className="flex gap-2"><button disabled={busy} className="px-3 py-2 rounded-lg bg-primary text-primary-foreground text-sm disabled:opacity-50">{busy ? "Saving…" : editing ? "Save changes" : "Add follow-up"}</button>{editing && <button type="button" onClick={() => { setEditing(null); setDraft(blank()); }} className="text-sm px-3">Cancel edit</button>}</div>
    </form>}
    <div className="flex flex-wrap items-center gap-2">
      <select aria-label="Follow-up view" value={view} onChange={(e) => { setView(e.target.value); setPage(0); }} className="rounded-lg border bg-background p-2 text-sm"><option value="day">By date</option><option value="overdue">Overdue</option><option value="all">All dates</option></select>
      {view === "day" && <input aria-label="Follow-up date" type="date" value={day} onChange={(e) => { if (e.target.value) chooseDay(e.target.value); }} className="rounded-lg border bg-background p-2 text-sm" />}
      <select aria-label="Follow-up status" value={status} onChange={(e) => { setStatus(e.target.value); setPage(0); }} className="rounded-lg border bg-background p-2 text-sm"><option value="pending">Pending</option><option value="done">Completed</option><option value="cancelled">Cancelled</option><option value="all">All statuses</option></select>
      <button onClick={() => setRevision((v) => v + 1)} className="text-xs border rounded-lg p-2">Refresh</button>
    </div>
    {error && <p role="alert" className="text-sm text-destructive">{error}</p>}
    {loading ? <p role="status" className="text-sm text-muted-foreground">Loading follow-ups…</p> : !error && <>
      {!items.length && <p className="border border-dashed rounded-xl p-6 text-sm text-muted-foreground">No follow-ups match this view.{!leadId && " Use Follow up on a lead in Records or Pipeline to add one."}</p>}
      {items.map((item) => <article key={item.id} className="rounded-xl border bg-card p-4 space-y-2">
        ...
        {item.outcome && <p className="text-sm rounded-lg bg-muted p-2"><strong>Outcome:</strong> {item.outcome}</p>}
        {item.status === "pending" && <label className="block text-xs">What happened?<input value={outcomes[item.id] || ""} maxLength={1000} onChange={(e) => setOutcomes((current) => ({ ...current, [item.id]: e.target.value }))} placeholder="Record the lead’s response before completing" className={`${inputClass} mt-1`} /></label>}
        <div className="flex flex-wrap gap-2 text-xs">{item.status === "pending" ? <><button disabled={busy || !outcomes[item.id]?.trim()} onClick={() => update(item.id, { status: "done", outcome: outcomes[item.id].trim() })} className="border rounded-lg px-3 py-2 disabled:opacity-50">Complete follow-up</button><button disabled={busy} onClick={() => { setEditing(item.id); setDraft({ title: item.title, note: item.objective || "", due_at: localTime(item.scheduled_time) }); }} className="border rounded-lg px-3 py-2">Edit / reschedule</button><button disabled={busy} onClick={() => update(item.id, { status: "cancelled" })} className="px-2 text-muted-foreground">Cancel follow-up</button></> : <button disabled={busy} onClick={() => update(item.id, { status: "pending" })} className="border rounded-lg px-3 py-2">Reopen</button>}</div>
      </article>)}
      <div className="flex items-center justify-between text-xs text-muted-foreground"><span>{total} follow-ups</span><div className="flex gap-3"><button disabled={!page} onClick={() => setPage((v) => v - 1)}>Previous</button><button disabled={(page + 1) * 50 >= total} onClick={() => setPage((v) => v + 1)}>Next</button></div></div>
    </>}
  </section>;
}
