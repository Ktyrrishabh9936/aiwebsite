...
const positiveCount = (value) => /^\d+$/.test(String(value)) && Number(value) > 0 && Number(value) <= 5000;
...
export default function Properties() {
  ...
  const generate = async (property) => {
    try {
      ...
      toast.success(`${data.generated_units} units ready to manage`);
      await load();
  ...
  return <div className="p-6 sm:p-10 max-w-7xl mx-auto space-y-8">
    ...
    {open && <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm p-4 overflow-y-auto" role="dialog" aria-modal="true" aria-label={editingId ? "Edit property" : "Add property"}><form onSubmit={create} className="max-w-3xl mx-auto my-8 rounded-xl border bg-card p-6 sm:p-8 space-y-7">
      ...
      </section> : <section className="space-y-4"><h3 className="font-semibold">How many {form.category === "land" ? "blocks" : "units"} are in this property?</h3><p className="text-sm text-muted-foreground">Each {form.category === "land" ? "block" : "unit"} will have its own availability and place in the plan view. Up to 5,000 units.</p>{form.category === "land" && <p className="text-sm text-muted-foreground">The starting price is used for each block. The site area describes the whole site.</p>}<div className="grid gap-3 sm:grid-cols-2 max-w-lg"><NumberField label={form.category === "land" ? "Number of blocks" : "Total units"} value={form.inventory_setup.total_units || ""} onChange={(value) => setInventory("total_units", value)} />{form.category === "land" && <label className="block space-y-1.5"><span className="text-xs font-semibold text-muted-foreground uppercase">Site area (sq ft, optional)</span><input type="number" min="0" step="0.01" value={form.attributes?.area_sqft || ""} onChange={(event) => setAttribute("area_sqft", event.target.value)} className="w-full h-10 px-3 rounded-lg border bg-background text-sm" /></label>}</div></section>)}
...
function PropertyCard({ property, currency, onEdit, onManage, onGenerate, onDelete }) {
  ...
  const unitNoun = property.subtype === "Apartment" ? "flats" : property.category === "land" ? "blocks" : "units";
  const action = generatedUnits > 0 && allGenerated ? "View inventory" : numbered || (!unitMix.length && plannedUnits > 0) ? `Generate ${plannedUnits} ${unitNoun}` : "Set flat numbers & prices";
  const openInventory = () => generatedUnits > 0 && allGenerated ? onManage(property) : numbered || (!unitMix.length && plannedUnits > 0) ? onGenerate(property) : onEdit(property);
  return <article className="rounded-xl border bg-card p-5 space-y-4">
    ...
    <div className="flex flex-wrap gap-x-5 gap-y-1 text-sm"><span>{property.subtype || property.category}</span>{unitMix.length > 0 && <span className="font-semibold">{generatedUnits} of {plannedUnits} flats ready · {towers} {towers === 1 ? "tower" : "towers"}</span>}{unitMix.length === 0 && plannedUnits > 0 && <span>{generatedUnits} of {plannedUnits} {unitNoun} ready</span>}</div>
